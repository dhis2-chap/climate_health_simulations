...
from config.SimulationConfig import Config, IndependentVariable, ExplanatoryVariable
from config.config_variations.ConfigParameters import ConfigParameters
...
if __name__ == "__main__":
    base_config = Config(
        ...
        dependent_variable=DependentVariable(
            ...
            explanatory_variables=[
                ExplanatoryVariable(type='rain', lag=3),
                ExplanatoryVariable(type='temperature', lag=3),
                ExplanatoryVariable(type='season')
            ],
            population=100
        ),
        n_time_points=10
    ...
    # Generate all valid configs with progress tracking
    total = 0
    valid = 0
    for config in ConfigGenerator.generate_all(base_config):
        total += 1
        valid += 1
        print(f"Generated valid config #{valid}")

    print(f"Success rate: {valid}/{total} ({valid / total:.1%})")